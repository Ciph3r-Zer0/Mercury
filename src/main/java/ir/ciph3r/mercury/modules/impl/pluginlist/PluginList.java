...
public class PluginList extends CommandModule {
    ...
    public void onPluginList(CommandSender sender) {
        ...
        ChatUtils.sendColorizedMSG(sender, MercuryAPI.INSTANCE.getMessages().PLUGIN_LIST_FIRST_LINE.replace("{amount}", String.valueOf(plugins.length)));

        for (Plugin pl : plugins) {
            ChatUtils.sendColorizedMSG(sender, MercuryAPI.INSTANCE.getMessages().PLUGIN_LIST_LIST_DESIGN
                    .replace("{plugin}", handlePluginActivity(pl))
                    .replace("{version}", pl.getDescription().getVersion()));
        }

        ChatUtils.sendColorizedMSG(sender, MercuryAPI.INSTANCE.getMessages().PLUGIN_LIST_LAST_LINE.replace("{amount}", String.valueOf(plugins.length)));
    }
