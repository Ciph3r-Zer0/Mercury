...
public class Teleport extends Model {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            Utils.sendColorizedMessage(player, getMessages().TELEPORT_USAGE);
            return true;
            ...
            if (target == null) {
                ...
                Utils.sendColorizedMessage(player, getMessages().TELEPORT_SUCCESS_PLAYER.replace("{player}", target.getName()));
                return true;
            }
            ...
            if (target1 == null) {
                ...
                Utils.sendColorizedMessage(player, getMessages().TELEPORT_SUCCESS_OTHERS.replace("{player}", target1.getName()).replace("{target}", target2.getName()));
                return true;
            }
            ...
            Utils.sendColorizedMessage(player, getMessages().TELEPORT_SUCCESS_LOCATION.replace("{x}", args[0]).replace("{y}", args[1]).replace("{z}", args[2]));
            return true;
            ...
            Utils.sendColorizedMessage(player, getMessages().TELEPORT_SUCCESS_OTHERS_LOCATION.replace("{player}", target.getName()).replace("{x}", args[1]).replace("{y}", args[2]).replace("{z}", args[3]));
            return true;
        }
        return true;
    }
